...
rob_forest_rates <-
  function(res,
    ...
    for (i in 1:nrow(dat_rob_vec)) {
      ...
      graphics::text(x_min, dat_rob_vec$heading[i] + 1, pos=4, dat_rob_vec[[group.var]][i], cex = 1)
    }
    ...
    #graphics::text(mean(header_row), y_max, labels = "Risk of Bias", cex=1)
    ...
    if (length(unique(dat[[group.var]]))>1 && nrow(dat)>1) {
      ...
      graphics::text(x_min,-1.8, pos = 4, cex = 0.8, bquote(
        paste(
